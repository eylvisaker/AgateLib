...
namespace AgateLib.DisplayLib
{
    ...
    public sealed class PixelBuffer
    {
        ...
        {
            Condition.Requires<ArgumentNullException>(buffer != null, "buffer");
            Condition.Requires<ArgumentOutOfRangeException>(srcRect.X >= 0 && srcRect.Y >= 0 && srcRect.Right <= buffer.Width && srcRect.Bottom <= buffer.Height, "srcRect");
            Condition.Requires<ArgumentOutOfRangeException>(destPt.X >= 0 && destPt.Y >= 0);
            Condition.Requires<ArgumentOutOfRangeException>(clip || (srcRect.Width + destPt.X <= Width), "Attempt to copy area to invalid region.");
            Condition.Requires<ArgumentOutOfRangeException>(clip || (srcRect.Height + destPt.Y <= Height), "Attempt to copy area to invalid region.");
